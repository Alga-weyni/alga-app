...
- **Mandatory Hardware Requirements**: All properties must have (1) TTLock smart lockbox with keypad (stores physical keys, unlocked via 4-digit PIN codes generated by backend API, works offline without Bluetooth/internet), (2) Security cameras (surveillance for guest safety and property protection). Properties cannot be approved without verified installation of both devices. **Check-in flow**: On booking confirmation, Alga backend calls TTLock Keyboard Password API to generate time-limited 4-digit code → Code sent to guest via SMS/app → Guest enters code on lockbox keypad → Retrieves physical key → Code auto-expires after checkout.
